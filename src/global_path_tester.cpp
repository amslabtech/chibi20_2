...
A_Star_Planner::A_Star_Planner() :private_n("~")
{
    ...
    sub_map_metadata=n.subscribe("map_metadata",1,&A_Star_Planner::map_metadata_callback,this);
...
void A_Star_Planner::map_metadata_callback(const nav_msgs::MapMetaData::ConstPtr& msg)
{
    map_metadata=*msg;
}

void A_Star_Planner::map_updater()
{
    // if(map_received)
    //     return;
    std::cout<<"map received"<<std::endl;
    // map_received=true;
    row=map_metadata.height;
    column=map_metadata.width;
    ...
    // }
    //###use in test only
    searching_grid.x=prior_map.info.origin.position.x;
    searching_grid.y=prior_map.info.origin.position.y;
    // std::cout<<searching_grid.x<<","<<searching_grid.y<<std::endl;
    //###
